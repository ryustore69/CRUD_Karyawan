        ...
        <div class="container mx-auto px-4 max-w-2xl">
            ...
            <div class="flex items-center gap-4 mb-6">
                ...
                <div>
                    ...
                    <p class="text-gray-600 text-sm">Perbarui data karyawan: <strong>{{ $karyawan->nama }}</strong></p>
                </div>
            </div>
                ...
                <form action="{{ route('karyawans.update', $karyawan->id) }}" method="POST">
                    ...
                    <div class="mb-6">
                        ...
                        <select id="pendidikan" 
                            ...
                            <option value="S1" {{ old('pendidikan', $karyawan->pendidikan) == 'S1' ? 'selected' : '' }}>S1 (Sarjana)</option>
                            <option value="S2" {{ old('pendidikan', $karyawan->pendidikan) == 'S2' ? 'selected' : '' }}>S2 (Magister)</option>
                            <option value="S3" {{ old('pendidikan', $karyawan->pendidikan) == 'S3' ? 'selected' : '' }}>S3 (Doktor)</option>
                        </select>
                        @error('pendidikan')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="mb-6">
                        ...
                        <select id="divisi" 
                            ...
                            <option value="Produksi" {{ old('divisi', $karyawan->divisi) == 'Produksi' ? 'selected' : '' }}>Produksi</option>
                            <option value="SDM" {{ old('divisi', $karyawan->divisi) == 'SDM' ? 'selected' : '' }}>SDM</option>
                            <option value="IT" {{ old('divisi', $karyawan->divisi) == 'IT' ? 'selected' : '' }}>IT (Information Technology)</option>
                            <option value="HRD" {{ old('divisi', $karyawan->divisi) == 'HRD' ? 'selected' : '' }}>HRD (Human Resources Development)</option>
                            <option value="Finance" {{ old('divisi', $karyawan->divisi) == 'Finance' ? 'selected' : '' }}>Finance</option>
                            <option value="Operations" {{ old('divisi', $karyawan->divisi) == 'Operations' ? 'selected' : '' }}>Operations</option>
                            <option value="Quality Control" {{ old('divisi', $karyawan->divisi) == 'Quality Control' ? 'selected' : '' }}>Quality Control</option>
                            <option value="Research & Development" {{ old('divisi', $karyawan->divisi) == 'Research & Development' ? 'selected' : '' }}>Research & Development</option>
                            <option value="Legal" {{ old('divisi', $karyawan->divisi) == 'Legal' ? 'selected' : '' }}>Legal</option>
                            <option value="Procurement" {{ old('divisi', $karyawan->divisi) == 'Procurement' ? 'selected' : '' }}>Procurement</option>
                            <option value="Customer Service" {{ old('divisi', $karyawan->divisi) == 'Customer Service' ? 'selected' : '' }}>Customer Service</option>
                        </select>
                        @error('divisi')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
            ...
            <div class="bg-gray-50 rounded-lg shadow-md p-6 mt-6">
                ...
                <ul class="text-sm text-gray-600 space-y-1">
                    <li><strong>ID Karyawan:</strong> #{{ $karyawan->id }}</li>
                    <li><strong>Dibuat pada:</strong> {{ $karyawan->created_at->format('d/m/Y H:i') }}</li>
                    <li><strong>Terakhir diupdate:</strong> {{ $karyawan->updated_at->format('d/m/Y H:i') }}</li>
                </ul>
            </div>
            ...
            <div class="bg-red-50 border-l-4 border-red-500 rounded-lg shadow-md p-6 mt-6">
                ...
                <form action="{{ route('karyawans.destroy', $karyawan->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus karyawan {{ $karyawan->nama }}? Tindakan ini tidak dapat dibatalkan!');">
                    @csrf
                    @method('DELETE')
